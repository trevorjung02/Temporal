...
class GPT2(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        steps_per_epoch = ( len_data // denominator ) + 1
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=self.hparams.learning_rate, steps_per_epoch=steps_per_epoch, pct_start=0.1, final_div_factor=4, epochs=self.hparams.num_train_epochs, anneal_strategy='linear', cycle_momentum=False)
    ...
    def train_dataloader(self):   
        ...
        dataloader = DataLoader(train_dataset,  batch_size=self.hparams.train_batch_size, drop_last=True, num_workers=self.hparams.num_workers, pin_memory=True)
        return dataloader
    ...
    def val_dataloader(self):
        if self.hparams.mode == 'pretrain':
            ...
            return DataLoader(validation_dataset, batch_size=self.hparams.eval_batch_size, num_workers=self.hparams.num_workers, shuffle=False, pin_memory=True)
    ...
    def test_dataloader(self):
        ...
        return DataLoader(test_dataset, batch_size=self.hparams.eval_batch_size, num_workers=self.hparams.num_workers, shuffle=False, pin_memory=True)