...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config):
        ...
        encoder_config.use_cache = False
        self.encoder = T5Stack(encoder_config, self.shared)
        encoder_config.update(adapter_config)
        self.enc_kadapters = nn.ModuleDict()
        for year,path in adapter_config['years_to_paths'].items():
            model = AdapterModel(encoder_config)
            if adapter_config['load_adapters']:
                model.load_state_dict(torch.load(path))
            self.enc_kadapters.update({year: model})
        print(self.enc_kadapters.keys())
        ...
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared)
        self.config.update({'adapter_enc_dec': adapter_config['adapter_enc_dec']})
        if self.config.adapter_enc_dec:
            decoder_config.update(adapter_config)
            self.dec_kadapter = AdapterModel(decoder_config)
    ...
    def forward(
        ...
        if encoder_outputs.hidden_states != None:
            # print(year)
            year = year[0].item()
            hidden_states = self.enc_kadapters[str(year)](encoder_outputs)
        else:
            hidden_states = encoder_outputs[0]
        ...
        if self.config.adapter_enc_dec:
            sequence_output = self.dec_kadapter(decoder_outputs)
        else:
...
class Adapter(nn.Module):
    def __init__(self, config):
        ...
        t5block_config = copy.deepcopy(config)
        t5block_config.d_model = config.adapter_hidden_size
        self.encoder = T5Block(t5block_config, has_relative_attention_bias=True)
...
class AdapterModel(nn.Module):
    def __init__(self, config):
        ...
        self.adapter = nn.ModuleList(
            [Adapter(self.config) for i in range(self.adapter_num)]
        )
        # self.adapter = nn.ModuleList(
        #     [T5Block(self.config, has_relative_attention_bias=True) for i in range(self.adapter_num)]
        # )

        # self.dense = nn.Linear(self.config.d_model * 2, self.config.d_model)
        self.pool = nn.Linear(2, 1)

    def forward(self, pretrained_model_outputs):
        # if self.config.is_decoder:
        #     print("Decoder Adapter")
        outputs = pretrained_model_outputs
        sequence_output = outputs.last_hidden_state
        hidden_states = outputs.hidden_states
        device = 'cuda:'+str(sequence_output.get_device())
        hidden_states_last = torch.zeros(
            hidden_states[1].size(), device=device)
        ...
        outputs = self.layer_norm(hidden_states_last)

        batch_size = hidden_states[1].shape[0]
        expand_size = sequence_output.shape[0] // batch_size

        if expand_size != 1:
            shrink_idx = torch.arange(0, sequence_output.shape[0], expand_size, device=sequence_output.device)
            # print(f"shrink_idx = {shrink_idx}")
            # print(f"sequence_output = {sequence_output}")
            sequence_output = sequence_output.index_select(0, shrink_idx)
            # print(f"sequence_output = {sequence_output}")

        # outputs = self.dense(torch.cat((outputs, sequence_output), dim=-1))
        # outputs = outputs + sequence_output
        outputs = torch.cat((outputs.unsqueeze(-1), sequence_output.unsqueeze(-1)), -1)
        outputs = self.pool(outputs).squeeze(-1)

        if expand_size != 1:
            expanded_return_idx = (
                torch.arange(batch_size).view(-1, 1).repeat(1,
                                                            expand_size).view(-1).to(outputs.device)
            )
            outputs = outputs.index_select(0, expanded_return_idx)
        return outputs
