...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if not self.is_decoder or config.adapter_enc_dec:
            adapter_config = copy.deepcopy(config)
            adapter_config.update({'d_ff': config.adapter_hidden_size})
            self.adapter = T5LayerFF(adapter_config)
            self.adapter_pooling = nn.Linear(2, 1)
    ...
    def forward(
        ...
        if not self.is_decoder or self.config.adapter_enc_dec:
            parallel_states = self.adapter(hidden_states)
            parallel_states = clamp_inf(parallel_states)
        ...
        if not self.is_decoder or self.config.adapter_enc_dec:
            hidden_states = torch.cat((hidden_states.unsqueeze(-1), parallel_states.unsqueeze(-1)), -1)
            hidden_states = self.adapter_pooling(hidden_states).squeeze(-1)
