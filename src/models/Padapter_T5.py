...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if encoder_outputs.hidden_states != None:
            hidden_states = self.enc_kadapter(encoder_outputs)
        else:
            hidden_states = encoder_outputs[0]
...
class Adapter(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print(f"down_projected size = {down_projected.size()}")
        outputs = self.relu(down_projected)
        # print(f"outputs size = {outputs.size()}")
        up_projected = self.up_project(outputs)
        # print(f"up_projected size = {up_projected.size()}")
