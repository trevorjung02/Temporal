...
class AdapterModel(nn.Module):
    def __init__(self, pretrained_config):
        ...
        # self.adapter_list = [1,2,3,4,5,6]
        self.adapter_list = [1,5]
        self.adapter_num = len(self.adapter_list)
        self.pool_size = 3
        self.ff = nn.Linear(self.pool_size, 1)
        self.layer_norm = T5LayerNorm(self.config.d_model, eps=self.config.layer_norm_epsilon)
        self.adapters = nn.ModuleList(
            [nn.ModuleList(
                [T5Block(self.config, has_relative_attention_bias=bool(i == 0)) for i in range(self.pool_size)]) 
            for j in range(self.adapter_num)]
        )
    ...
    def forward(self, pretrained_model_outputs):
        ...
        hidden_states_last = torch.zeros((self.pool_size,) + sequence_output.size(), device=device)
        # print(f"sequence_output: {sequence_output.size()}")
        # print(f"hidden_states_last: {hidden_states_last.size()}")

        for i in range(self.adapter_num):
            pretrained_hidden_state = hidden_states[self.adapter_list[i]]
            for j, adapter_module in enumerate(self.adapters[i]):
                fusion_state = pretrained_hidden_state + hidden_states_last[j]
                hidden_states_last[j] = adapter_module(fusion_state)[0]

        transposed = hidden_states_last.permute(1,2,3,0)
        # print(f"transposed: {transposed.size()}")
        pooled_outputs = self.ff(transposed)
        # print(f"pooled_outputs: {pooled_outputs.size()}")
        pooled_outputs = pooled_outputs.squeeze()
        # print(f"pooled_outputs squeezed: {pooled_outputs.size()}")
        scale_factor = 0.1
